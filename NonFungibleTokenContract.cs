...
public class NonFungibleTokenContract : SmartContract
{
    /// <summary>
    ...
    /// <summary>
    /// The end block to restrict the minting NFT after reaching the end block(duration for minting)
    /// </summary>
    ...
    /// <summary>
    /// For number of NFT in a Contract
    /// </summary>
    private UInt256 GetSenderBalance(Address addresssender) => State.GetUInt256($"Balance:{addresssender}");


    private void SetSenderBalance(Address addresssender, UInt256 value) => State.SetUInt256($"Balance:{addresssender}", value);
    ...
    /// </param>
    public NonFungibleTokenContract(ISmartContractState state, string name, string symbol, bool ownerOnlyMinting, ulong duration) : base(state)
    {
        ...
        this.EndBlock = this.Block.Number + duration;
    }
    ...
    /// <summary>
    /// Returns the number of NFTs minted by a 'Address'. 
    /// </summary>
    /// <param name="sender">Address for whom to query the balance.</param>
    /// <returns>Balance of owner.</returns>
    public UInt256 BalanceOfSender(Address sender)
    {
        EnsureAddressIsNotEmpty(sender);
        return GetSenderBalance(sender);
    }
    ...
    private void RemoveToken(Address from, UInt256 tokenId)
    {
        ...
        //remove from Sender
        var senderTokencount = GetSenderBalance(Message.Sender);
        SetSenderBalance(Message.Sender, senderTokencount - 1);
    }
    ...
    private void AddToken(Address to, UInt256 tokenId)
    {
        ...
        //Setting Balance of sender
        var currentSenderTokenAmount = GetSenderBalance(Message.Sender);
        SetSenderBalance(Message.Sender, currentSenderTokenAmount + 1);
    }
    ...
    {
        Assert(this.Block.Number < this.EndBlock, "Minting duration is finished!");
        var currentTokenAmount = GetBalance(to);
        Assert(currentTokenAmount < 1, "Only one NFT allowed to mint for one address!");
        UInt256 numberofNFT = GetSenderBalance(Message.Sender);
        Assert(numberofNFT < 6, "Allowed to mint max 5 NFTs");
